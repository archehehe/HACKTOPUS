...
# Setup logging
logging.basicConfig(filename='speak4mepro.log', level=logging.DEBUG, 
...
class Speak4MePro:
    def __init__(self, root):
        ...
        # Select default voice based on content type suitability
        self.select_default_voice()
    ...
    def initialize_engine(self):
        """Initialize or reinitialize the speech engine with error handling"""
        try:
            self.engine = pyttsx3.init()
            logging.info("Speech engine initialized successfully")
        except Exception as e:
            logging.error(f"Failed to initialize speech engine: {str(e)}")
            messagebox.showerror("Engine Error", f"Failed to initialize speech engine:\n{str(e)}")
            self.engine = None

    def select_default_voice(self):
        """Select a default voice, preferring female or clear voices"""
        if not self.voices:
            return
    ...
    def setup_ui(self):
        ...
        contexts = ['Auto', 'News', 'Story', 'Technical', 'Dialogue', 'Poetry']
    ...
    def update_engine_settings(self):
        if not self.engine:
            self.initialize_engine()
            if not self.engine:
                self.update_status("Speech engine unavailable")
                return
        ...
        try:
            ...
            # Apply context-specific settings (context overrides emotion)
            if self.context_mode != 'Auto':
                self.apply_context_style()
            elif self.emotion_mode != 'Normal':
                self.apply_emotion(self.emotion_mode)
            
            # Apply learning mode last to ensure slower rate if enabled
            if self.learning_mode:
                self.engine.setProperty('rate', max(80, self.rate - 50))
            
            logging.info(f"Engine settings: voice={self.voices[self.current_voice].name}, rate={self.rate}, volume={self.volume}, context={self.context_mode}")
        except Exception as e:
            logging.error(f"Failed to update engine settings: {str(e)}")
            messagebox.showerror("Engine Error", f"Failed to update speech settings:\n{str(e)}")
    ...
    def update_rate(self, rate):
        ...
        self.update_engine_settings()
        self.update_status(f"Speech rate set to {rate} wpm")
        logging.info(f"Rate set to {rate} wpm")
        self.preview_voice()
    ...
    def update_volume(self, volume):
        ...
        self.update_engine_settings()
        self.update_status(f"Volume set to {int(volume*100)}%")
        logging.info(f"Volume set to {int(volume*100)}%")
        self.preview_voice()
    ...
    def speak_text(self, text):
        ...
        for i, word in enumerate(words):
            ...
            try:
                ...
                logging.error(f"Speech error for word '{word}': {str(e)}")
                self.update_status(f"Speech error: {str(e)}")
                break
    ...
    def open_file(self):
        ...
        if filepath:
            try:
                ...
                logging.info(f"File loaded: {filepath}")
            except Exception as e:
    ...
    def save_as_mp3(self):
        ...
        if filepath:
            try:
                ...
                logging.info(f"MP3 saved: {filepath}")
            except Exception as e:
                logging.error(f"Failed to save MP3: {str(e)}")
    ...
    def read_clipboard(self):
        try:
            ...
            if clipboard_text.strip():
                ...
                self.detect_content_type()
                logging.info("Clipboard content loaded")
            else:
    ...
    def load_presets(self):
        try:
            if os.path.exists('speak4me_presets.json'):
                ...
                logging.info("Presets loaded")
        except Exception as e:
            logging.warning(f"Could not load presets: {str(e)}")
    ...
    def save_presets_to_file(self):
        try:
            ...
            logging.info("Presets saved")
        except Exception as e:
            logging.warning(f"Could not save presets: {str(e)}")
    ...
    def save_preset(self):
        ...
        if preset_name:
            ...
            logging.info(f"Preset saved: {preset_name}")
    ...
    def load_preset(self):
        ...
        if selected >= 0 and selected < len(self.saved_presets):
            ...
            logging.info(f"Preset loaded: {preset['name']}")
            self.preview_voice()
    ...
    def apply_emotion(self, emotion):
        ...
        if emotion in emotions and self.context_mode == 'Auto':
            ...
            try:
                ...
                try:
                    ...
                    logging.warning("Pitch setting not supported")
                logging.info(f"Emotion applied: {emotion}, rate={self.rate}, volume={settings['volume']}")
            except Exception as e:
                logging.error(f"Failed to apply emotion: {str(e)}")
                messagebox.showerror("Error", f"Failed to apply emotion:\n{str(e)}")
    ...
    def toggle_learning_mode(self):
        ...
        self.update_engine_settings()
        self.update_status("Learning mode: Slower speech" if self.learning_mode else "Learning mode off")
        logging.info(f"Learning mode {'enabled' if self.learning_mode else 'disabled'}")

    def toggle_pronunciation_helper(self):
        self.pronunciation_helper = self.pronunciation_helper_var.get()
        self.update_status(f"Pronunciation helper {'ON' if self.pronunciation_helper else 'OFF'}")
        logging.info(f"Pronunciation helper {'ON' if self.pronunciation_helper else 'OFF'}")
    ...
    def add_custom_pronunciation(self):
        ...
        if word:
            ...
            if pronunciation:
                ...
                logging.info(f"Custom pronunciation added: {word} -> {pronunciation}")
    ...
    def load_pronunciation_dict(self):
        try:
            if os.path.exists('custom_pronunciations.json'):
                ...
                logging.info("Pronunciation dictionary loaded")
        except Exception as e:
            logging.warning(f"Could not load pronunciations: {str(e)}")
    ...
    def save_pronunciation_dict(self):
        try:
            ...
            logging.info("Pronunciation dictionary saved")
        except Exception as e:
            logging.warning(f"Could not save pronunciations: {str(e)}")
    ...
    def detect_content_type(self):
        ...
        text = self.text_input.get("1.0", tk.END)[:1000].lower()
        if re.search(r'\b(poem|verse|stanza|rhyme|sonnet|haiku)\b', text):
            self.context_var.set('Poetry')
        elif re.search(r'\b(chapter|story|tale|once upon|novel|fiction)\b', text):
            self.context_var.set('Story')
        elif re.search(r'\b(research|data|analysis|technical|study|experiment)\b', text):
            self.context_var.set('Technical')
        elif re.search(r'\b(said|replied|asked|dialogue|conversation|"|\')\b', text):
            self.context_var.set('Dialogue')
        elif re.search(r'\b(news|report|breaking|headline|journalist|article)\b', text):
            self.context_var.set('News')
        else:
            self.context_var.set('News')  # Default to News for neutral text
        self.apply_context_style()
        self.update_status(f"Detected content type: {self.context_var.get()}")
        logging.info(f"Content type detected: {self.context_var.get()}")

    def apply_context_style(self, *args):
        """Apply distinct speech settings for each content type"""
        ...
        context_styles = {
            'News': {
                'rate': 180,  # Fast, clear for professional delivery
                'pitch': 100,  # Neutral pitch
                'volume': 1.0,  # Full volume for clarity
                'voice_preference': ['david', 'male', 'zira']  # Prefer authoritative voices
            },
            'Story': {
                'rate': 140,  # Moderate pace for engagement
                'pitch': 95,  # Slightly lower for warmth
                'volume': 0.9,  # Softer for immersive feel
                'voice_preference': ['female', 'zira']  # Expressive voices
            },
            'Technical': {
                'rate': 150,  # Steady pace for clarity
                'pitch': 105,  # Slightly higher for precision
                'volume': 1.0,  # Clear and loud
                'voice_preference': ['david', 'male', 'mark']  # Clear, neutral voices
            },
            'Dialogue': {
                'rate': 160,  # Conversational pace
                'pitch': 110,  # Dynamic pitch for character
                'volume': 0.95,  # Slightly softer for naturalness
                'voice_preference': ['female', 'male']  # Varied voices
            },
            'Poetry': {
                'rate': 100,  # Slow, expressive pace
                'pitch': 90,  # Lower pitch for emotional depth
                'volume': 0.9,  # Softer for intimacy
                'voice_preference': ['female', 'zira']  # Expressive voices
            }
        }
        ...
        if context in context_styles:
            ...
            preferred_voices = settings['voice_preference']
            for i, voice in enumerate(self.voices):
            ...
            try:
                ...
                try:
                    ...
                    logging.warning(f"Pitch setting not supported for {context}")
                
                # Reapply learning mode if active
                if self.learning_mode:
                    self.engine.setProperty('rate', max(80, self.rate - 50))
                
                logging.info(f"Applied context: {context}, voice={self.voices[self.current_voice].name}, rate={self.rate}, volume={self.volume}")
                self.update_status(f"Set {context} mode: {self.rate} wpm, {int(self.volume*100)}% volume")
            except Exception as e:
                logging.error(f"Failed to apply context {context}: {str(e)}")
                messagebox.showerror("Error", f"Failed to apply {context} settings:\n{str(e)}")
    ...
    def create_voice_profile(self):
        ...
        if name:
            ...
            logging.info(f"Voice profile created: {name}")
    ...
    def apply_voice_profile(self):
        ...
        if selected >= 0 and selected < len(self.voice_profiles):
            ...
            self.context_mode = profile.get('context', 'Auto')
            ...
            logging.info(f"Voice profile loaded: {profile['name']}")
            self.preview_voice()
    ...
    def load_voice_profiles(self):
        try:
            if os.path.exists('voice_profiles.json'):
                ...
                logging.info("Voice profiles loaded")
        except Exception as e:
            logging.warning(f"Could not load voice profiles: {str(e)}")
    ...
    def save_voice_profiles(self):
        try:
            ...
            logging.info("Voice profiles saved")
        except Exception as e:
            logging.warning(f"Could not save voice profiles: {str(e)}")
    ...
    def toggle_accessibility(self, feature):
        ...
        logging.info(f"Accessibility feature {feature} {status}")
    ...
    def open_document(self):
        ...
        if filepath:
            try:
                ...
                self.detect_content_type()
                logging.info(f"Document loaded: {filepath}")
            except Exception as e:
                logging.error(f"Failed to read document: {str(e)}")
    ...
    def toggle_voice_commands(self):
        ...
        logging.info(f"Voice commands {'enabled' if self.voice_command_mode else 'disabled'}")
            ...
            try:
                ...
                for cmd, action in commands.items():
                    if cmd in command:
                        ...
                        logging.info(f"Voice command executed: {cmd}")
                        break
            except sr.WaitTimeoutError:
                continue
            except sr.UnknownValueError:
                self.update_status("Could not understand command")
                logging.warning("Voice command not understood")
                continue
            except Exception as e:
    ...
    def toggle_dark_mode(self):
        ...
        logging.info("Dark mode " + ("enabled" if self.dark_mode else "disabled"))
...
- Content Detection: Auto-adjusts for news, stories, poetry, etc.
...
- Optimized content types: News (authoritative), Story (engaging), Poetry (expressive), etc."""
