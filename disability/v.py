...
class Speak4MePro:
    def __init__(self, root):
        ...
        # Initialize engine with best available driver
    ...
    def initialize_engine(self):
        """Initialize the TTS engine with the best available driver"""
        try:
            if platform.system() == 'Windows':
                self.engine = pyttsx3.init(driverName='sapi5')  # Best for Windows
            elif platform.system() == 'Darwin':
                self.engine = pyttsx3.init(driverName='nsss')  # Best for Mac
            else:
                self.engine = pyttsx3.init(driverName='espeak')  # Better for Linux
            ...
            try:
                ...
                messagebox.showerror("Error", f"Could not initialize speech engine: {str(e)}")
                raise
    ...
    def setup_ui(self):
        ...
        contexts = ['Auto', 'News', 'Story', 'Technical', 'Dialogue', 'Poetry', 'Instruction']
    ...
    def start_speech(self):
        ...
        # Apply content type processing
        text = self.process_for_content_type(text)
        
        # Start speech in a separate thread
        threading.Thread(target=self.speak_text, args=(text,), daemon=True).start()
        self.update_status("Speaking...")
    
    def process_for_content_type(self, text):
        """Process text based on selected content type"""
        context = self.context_var.get()
        
        if context == 'News':
            # Add news-style pauses and emphasis
            text = text.replace('.', '. ').replace(',', ', ')
            text = re.sub(r'([A-Z][A-Za-z\s]+:)', r'\n\1', text)  # Format speaker names
        elif context == 'Technical':
            # Slow down for technical terms
            text = re.sub(r'([A-Z][a-z]+[A-Z][a-zA-Z]+)', r' \1 ', text)  # Add spaces around CamelCase
        elif context == 'Dialogue':
            # Add dramatic pauses between lines
            text = text.replace('"', '').replace("'", '')
            text = re.sub(r'(\n\s*\n)', r'\1... ', text)
        elif context == 'Poetry':
            # Add longer pauses at line breaks
            text = text.replace('\n', '\n... ... \n')
        elif context == 'Instruction':
            # Add numbered steps and pauses
            if not text.startswith(('1.', 'Step 1', 'First,')):
    ...
    def speak_text(self, text):
        ...
        def update_progress(i):
            ...
            self.command_queue.put(lambda: self.progress.configure(value=progress))
        ...
        for i, word in enumerate(words):
            ...
            if self.learning_mode:
                self.command_queue.put(lambda: self.highlight_current_word(word, start_pos, end_pos))
            
            # Add emotional pauses if needed
            if self.emotion_mode == 'Sad' and i % 5 == 0:
                self.engine.say(word + "...")
            elif self.emotion_mode == 'Angry' and word.endswith(('.', '!', '?')):
                self.engine.say(word + "!")
            else:
                self.engine.say(word)
        ...
        self.command_queue.put(lambda: self.progress.configure(value=0))
        if not self.paused:
            self.is_speaking = False
            self.command_queue.put(lambda: self.play_btn.configure(text="Speak"))
    ...
    def preview_voice(self):
        ...
        # Create distinctive preview text based on settings
        preview_text = ""
        if self.emotion_mode != 'Normal':
            preview_text += f"[{self.emotion_mode} voice] "
        
        context = self.context_var.get()
        if context != 'Auto':
            preview_text += f"[{context} style] "
        
        # Add context-specific demo text
        if context == 'News':
            preview_text += "This is a breaking news update. The event occurred earlier today."
        elif context == 'Technical':
            preview_text += "The technical specifications indicate a maximum throughput of 1.2 gigabits per second."
        elif context == 'Dialogue':
            preview_text += "John said: Hello there. Mary replied: How are you doing today?"
        elif context == 'Poetry':
            preview_text += "The woods are lovely, dark and deep. But I have promises to keep."
        elif context == 'Instruction':
            preview_text += "Step 1: Remove the cover. Step 2: Insert the battery. Step 3: Replace the cover."
        else:
            preview_text += "This is how I sound with the current settings."
        
        # Insert preview text at cursor position
        self.text_input.insert(tk.INSERT, f" {preview_text}")
    ...
    def save_as_mp3(self):
        ...
        if filepath:
            try:
                ...
                # Use gTTS with additional parameters for better quality
                tts = gTTS(text=text, lang='en', slow=False, 
                          lang_check=True, tld='com')  # Using .com domain for more natural voice
                tts.save(filepath)
    ...
    def load_preset(self):
        ...
        if selected >= 0 and selected < len(self.saved_presets):
            ...
            self.context_var.set(preset.get('context', 'Auto'))
    ...
    def apply_emotion(self, emotion):
        ...
        if emotion in emotions:
            ...
            # Adjust rate more dramatically for emotions
            ...
            try:
                ...
                # Compensate with rate changes
                if emotion == 'Happy':
                    self.engine.setProperty('rate', settings['rate'] + 20)
                elif emotion == 'Sad':
                    self.engine.setProperty('rate', settings['rate'] - 20)
            
            # For Windows SAPI5 voices, we can add pauses for effect
            if 'sapi5' in self.engine.getProperty('voices')[self.current_voice].id.lower():
                if emotion == 'Sad':
                    self.engine.setProperty('rate', settings['rate'] - 30)
                elif emotion == 'Angry':
                    self.engine.setProperty('rate', settings['rate'] + 30)
    ...
    def toggle_learning_mode(self):
        ...
        if self.learning_mode:
            ...
            self.engine.setProperty('rate', max(100, self.rate - 50))
            # Also try to make voice clearer if possible
            try:
                self.engine.setProperty('voice', self.voices[self.current_voice].id)
            except:
                pass
    ...
    def detect_content_type(self):
        ...
        # Check for poetry (short lines, rhyming patterns)
        lines = text.split('\n')
        if len(lines) > 3 and all(len(line) < 60 for line in lines[:5]):
            avg_line_length = sum(len(line) for line in lines[:5]) / 5
            if avg_line_length < 40:
                self.context_var.set('Poetry')
                return
        
        # Check for dialogue
        if any(keyword in text.lower() for keyword in ['said', 'replied', 'asked', 'answered']):
            self.context_var.set('Dialogue')
            return
        
        # Check for technical content
        if any(keyword in text.lower() for keyword in ['research', 'data', 'analysis', 'study', 'method', 'results']):
            self.context_var.set('Technical')
            return
        
        # Check for instructions
        if any(keyword in text.lower() for keyword in ['step', 'first', 'next', 'then', 'finally']):
            self.context_var.set('Instruction')
            return
        
        # Check for story
        if any(keyword in text.lower() for keyword in ['chapter', 'story', 'tale', 'once upon']):
            self.context_var.set('Story')
            return
        
        # Default to news style
    ...
    def apply_context_style(self, *args):
        ...
        context_styles = {
            'News': {
                ...
                'pitch': 100,
                'prefix': "",
                'style': lambda text: text.replace('.', '. ').replace(',', ', ')
            },
            'Story': {
                'rate': 130, 
                'volume': 0.9, 
            ...
            'Technical': {
                ...
                'pitch': 105,
                'prefix': "",
                'style': lambda text: re.sub(r'([A-Z][a-z]+[A-Z][a-zA-Z]+)', r' \1 ', text)
            },
            'Dialogue': {
                'rate': 160, 
                'volume': 1.0, 
            ...
            'Poetry': {
                ...
                'pitch': 95,
                'prefix': "",
                'style': lambda text: text.replace('\n', '\n... ... \n')
            },
            'Instruction': {
                'rate': 140,
                'volume': 1.0,
        ...
        if context in context_styles:
            ...
            processed_text = settings['style'](current_text)
            self.text_input.delete("1.0", tk.END)
            self.text_input.insert("1.0", processed_text)
    ...
    def apply_voice_profile(self):
        ...
        if selected >= 0 and selected < len(self.voice_profiles):
            ...
            self.context_var.set(profile.get('context', 'Auto'))
    ...
    def listen_for_commands(self):
        r = sr.Recognizer()
        r.pause_threshold = 0.8  # Reduce pause threshold for quicker response
        ...
        with sr.Microphone() as source:
            ...
            while self.voice_command_mode:
                try:
                    # Only listen if enough time has passed since last command
                    ...
                    try:
                        ...
                        commands = {
                            ...
                            'happy voice': lambda: self.apply_emotion('Happy'),
                            'sad voice': lambda: self.apply_emotion('Sad'),
                            'angry voice': lambda: self.apply_emotion('Angry'),
                            'calm voice': lambda: self.apply_emotion('Calm'),
                            'normal voice': lambda: self.apply_emotion('Normal'),
                            'news style': lambda: self.apply_context_style('News'),
                            'story style': lambda: self.apply_context_style('Story'),
                            'technical style': lambda: self.apply_context_style('Technical'),
                            'dialogue style': lambda: self.apply_context_style('Dialogue')
                        }
                        ...
                        for cmd, action in commands.items():
                            if cmd in command:
                                ...
                                threading.Thread(
                                    target=lambda: (
                                        self.engine.say(f"Executing {cmd}"),
                                        self.engine.runAndWait()
                                    ),
                                    daemon=True
                                ).start()
    ...
    def create_mini_mode(self):
        ...
        def speak_and_close():
            text = entry.get()
            if text:
                # Apply current settings to quick speak
                ...
                if self.emotion_mode != 'Normal':
                    self.apply_emotion(self.emotion_mode, engine=temp_engine)
...
2. Advanced Features:
- Emotion Mode: Change voice tone (Happy, Sad, Angry, Calm)
- Learning Mode: Slower, clearer speech with word highlighting
- Pronunciation Helper: Add custom pronunciations
- Voice Profiles: Save favorite voice settings
- Content Detection: Auto-adjusts for different text types (News, Story, Technical, Dialogue, Poetry, Instruction)

3. Voice Commands:
Enable voice commands to control the app hands-free. Supported commands:
- "Start reading", "Stop reading", "Pause reading", "Resume reading"
- "Increase speed", "Decrease speed", "Louder", "Quieter"
- Emotion commands: "Happy voice", "Sad voice", etc.
- Style commands: "News style", "Story style", etc.
...
- More distinct voice styles for different content
